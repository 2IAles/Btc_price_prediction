"""
Exemple d'utilisation du BTCPredictor.

Prérequis :
  1. conda activate projetml && python collect_daily.py
  2. python 02_preprocessing.py
  3. Exécuter 03_models.ipynb (entraînement)
  4. Exécuter 04_results_analysis.ipynb (sauvegarde best_model.pkl)

Ensuite :
  conda activate projetml && python example_usage.py
"""
...
if not non_empty_frames:
    raise RuntimeError(
        "Aucun fichier daily trouvé dans data/. "
        "Exécutez collect_daily.py d'abord."
    )
...
merged = merged.ffill()
print(f"Données disponibles : {merged.index.min().date()} → {merged.index.max().date()}")
...
print("=" * 60)
print(f"  Direction   : {'HAUSSE' if result['direction'] == 1 else 'BAISSE'} ({result['direction']})")
print(f"  Probabilité : {result['probability']:.4f}  ({result['probability']*100:.1f}%)")
