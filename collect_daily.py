...
def update_btc(verbose: bool = True) -> None:
    ...
    today = date.today()
    yf_start = (
        (local_max + timedelta(days=1)).strftime("%Y-%m-%d")
        if local_max
        else "2012-01-01"
    )
...
def update_xau(verbose: bool = True) -> None:
    ...
    local_after = (
        max(last, local_floor - timedelta(days=1))
        if last
        else local_floor - timedelta(days=1)
    )
...
def update_mvrv_nupl(verbose: bool = True) -> None:
    ...
    start = (
        (last + timedelta(days=1)).strftime("%Y-%m-%dT00:00:00Z")
        if last
        else "2012-01-01T00:00:00Z"
    )
...
def _fetch_trends_window(pt, ws: date, we: date, retries: int = 3) -> pd.DataFrame:
    ...
    for attempt in range(retries):
        try:
            ...
            wait = 10 * (attempt + 1)
            print(
                f"    window {ws}→{we} attempt {attempt+1} failed: {e}. Retrying in {wait}s …"
            )
            time.sleep(wait)
    return pd.DataFrame()
...
def update_google_trends(verbose: bool = True) -> None:
    ...
    if verbose:
        print(
            f"  [TRENDS] fetching full history 2012→{today} (monthly, single request) …"
        )
